...
if (!resolved) {
  ...
  if (fs.existsSync(srcEntry)) {
    ...
    if (bun.error) {
      console.error(
        "No compiled binary found and 'bun' is not available.\n" +
        "Install bun (https://bun.sh) or run: npm install -g opencode-" + platform + "-" + arch,
      )
      process.exit(1)
    }
    process.exit(typeof bun.status === "number" ? bun.status : 0)
  }

  console.error(
    "It seems that your package manager failed to install the right version of the ultron CLI for your platform. You can try manually installing " +
