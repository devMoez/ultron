    ...
    execute: (params: z.infer<typeof Parameters>, ctx: Tool.Context) =>
      Effect.gen(function* () {
        ...
        if (platform === "win32") {
          ...
          yield* Effect.tryPromise(() => execAsync(cmds[params.action] ?? `echo "unsupported on macOS"`)).pipe(Effect.orDie)
        } else {
          const cmds: Record<string, string> = {
          ...
          yield* Effect.tryPromise(() => execAsync(cmds[params.action] ?? `echo "unsupported"`)).pipe(Effect.orDie)
        }
