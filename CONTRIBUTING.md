...
Welcome to the Ultron ecosystem. This project is a specialized evolution of OpenCode, designed for advanced desktop automation and agentic workflows.

We value contributions that align with our vision of a high-performance, autonomous AI assistant. Here are the types of changes we prioritize:

- **Desktop Integration**: Enhancements to Windows automation and system tools.
- **Swarm Intelligence**: Improvements to multi-agent orchestration and routing.
- **Memory Systems**: Refinements to persistent context and learned behaviors.
- **Performance**: Optimizations for Bun-based execution and LLM inference.
- **Stability**: Fixes for environment-specific quirks and edge cases.

## Development Workflow

Ultron is built on **Bun 1.3+**. To get started:
...
By default, `bun dev` runs Ultron in the `packages/opencode` directory. To run it against a different directory or repository:

```bash
bun dev <directory>
```

To run Ultron in the root of the repo itself:
...
```bash
./packages/opencode/dist/ultron-<platform>/bin/ultron
```
